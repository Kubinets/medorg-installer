...
# Выбор модулей (ИСПРАВЛЕННАЯ ВЕРСИЯ С ПРОВЕРКОЙ ВВОДА)
select_modules() {
    ...
    # Проверяем, можем ли мы читать ввод (не пайп)
    if [[ ! -t 0 ]]; then
        # Не можем читать ввод (пайп) - используем только обязательные
    ...
    # Только здесь - интерактивный режим с доступным вводом
